...
class SparseFoolAttack(BatchAttack):
    ...
    @generator_decorator
    def as_generator(self, a, steps=30, lambda_=1.0, subsample=10):
        ...
        for step in range(steps):
            ...
            boundary_point, boundary_normal = yield from self.boundary_approximation_deepfool(
                a, perturbed, subsample, _label, lambda_
            )
            ...
            perturbation = self.l1_linear_solver(
                perturbed, boundary_point, boundary_normal, min_, max_
            )
    ...
    @classmethod
    def boundary_approximation_deepfool(
        cls, a, initial_point, subsample, label, lambda_, steps=100
    ):
        ...
        # define labels
        logits, _ = yield from a.forward_one(initial_point)
        ...
        def get_residual_labels(logs):
            ...
            return [k for k in labels if logs[k] < logs[label]]
        ...
        for step in range(steps):
            ...
            if is_adv:
                ...
                grad_fool = yield from a.gradient_one(
                    boundary_point, label=fooling_label, strict=False
                )
                grad_true = yield from a.gradient_one(
                    boundary_point, label=label, strict=False
                )
            ...
            for k in residual_labels:
                grad = yield from a.gradient_one(boundary_point, label=k, strict=False)
                grads.append(grad)
    ...
    def l1_linear_solver(cls, initial_point, boundary_point, normal, min_, max_):
        ...
        while current_sign == sign_true and np.count_nonzero(coordinates) > 0:
            ...
            mask[
                np.unravel_index(np.argmax(np.absolute(coordinates)), coordinates.shape)
            ] = 1

            perturbation = (
                max(abs(f_k) / np.amax(np.absolute(coordinates)), 1e-4)
                * mask
                * np.sign(coordinates)
            )
