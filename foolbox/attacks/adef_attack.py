...
def _transpose_image(image):
    # transpose the image so the color axis is at the front: image.shape is then cxhxw:
    return np.transpose(image,(2,0,1))

def _re_transpose_image(image):
    # transpose the image back so the color axis is at the end: image.shape is then hxwxc:
    return np.transpose(image,(1,2,0))

def _difference_map( image, color_axis ):
    """
    Difference map of the image
    Approximate derivatives of the function image[c,:,:] (e.g. PyTorch) or image[:,:,c] (e.g. Keras).
    ...
    ########################################################################################
    Note from Evgenia: I am not sure why they did not use the function np.gradient for this. 
    It seems to give very similar results. I have let it at the original implementation, though.
    ########################################################################################
...
class ADefAttack(Attack):
    """
    Adversarial Attack that distorts the image, i.e. changes the locations of pixels. The algorithm is described in [1],
    ...
    To access the vectorfields, an additional attribute "result_data" (type=dict) is 
    created 'on the fly' after the image is detected as being "adversarial" and 
    before "return". It can be accessed from the main program via attack.result_data. 
    Currently, it is not initialized before it is created (which is maybe not very nice), 
    because I did not know whether this is actually desired, 
    since it is only required and thus created for this particular attack. Also, I was not sure
    ...
    ----------
      [1] Rima Alaifari, Giovanni S. Alberti, and Tandri Gauksson1:
           ...
           https://arxiv.org/pdf/1804.07729.pdf
    ...
    def __call__(self, input_or_adv, ind_of_candidates = 1, unpack=True,
        ...
        # Number of classes to target + 1 (>=2). # Example 1: num_classes = 10 for MNIST
        # Example 2: If ind_of_candidates == 1, then only the second highest label is targeted
        ...
        for step in range(max_iter):
            ...
            # in a special way for PyTorch. In FoolBox, this is an internal private method
            # that is only used when e.g. the gradients w.r.t. the image are 
            # calculated. Here, we need this preprocessed image explicitly
            # to calculate the difference map.
            d1x, d2x = _difference_map( _preprocess_image(perturbed, preprocessing, color_axis), color_axis )          

            logits_for_grad=np.zeros_like(logits)
            logits_for_grad[original_label[0]] = 1
            grad_original = _deprocess_gradient( a.backward(logits_for_grad, perturbed), preprocessing, color_axis )
            
            # Find vector fields for the image and each candidate label.
            # Keep the smallest vector field for each image.
            norm_min = np.inf
            
            for target_no in range( 1, num_classes ): # iterate over all candidate classes
                
                target_labels = candidates[ 0, target_no ]
                logits_for_grad=np.zeros_like(logits)
                logits_for_grad[target_labels] = 1   
                # gradient of the target label w.r.t. image
                grad_target = _deprocess_gradient( a.backward(logits_for_grad, perturbed), preprocessing, color_axis )
                ...
                tau_target = _create_tau( f_im, dfx, d1x, d2x, color_axis, smooth ) # create the vector field
                
                tau_target += tau_full
                
                norm_target = np.linalg.norm( tau_target.ravel() ) # l2 norm of vector field.
                
                if norm_target < norm_min:  #choose the vector field with the smallest norm
                    norm_min = norm_target
                    tau_min = tau_target
            
            # Update the image by applying the vector field, the vector field is always applied to the original image,
            # since the current vector field is added to all prior vector fields via
            # tau_target += tau_full
            perturbed_processed = _compose(  _preprocess_image(image_original.copy(), preprocessing, color_axis), tau_min, color_axis )
            perturbed = _deprocess_image(perturbed_processed, preprocessing, color_axis)             
            tau_full = tau_min
            norm_full= norm_min
            
            fx, _ = a.predictions(perturbed)  # getting the current label after applying the vector field
            ...
            if targeting and (current_label in candidates[0,1:]):
                vprint('Image successfully deformed from %d to %d.' % (original_label, current_label ))
                continue
        ...
        if len(predicted_labels) > 1:
            vprint('\tDeformed image lies on the decision boundary of labels: ' + str(predicted_labels) )
            try_overshoot = True
        ...
        try_overshoot = try_overshoot or (targeting and not (current_label in candidates[0,1:]))
        ...
        if try_overshoot: 
            ...
            perturbed_processed = _compose( _preprocess_image(image_original.copy(), preprocessing, color_axis), tau_full, color_axis )
            perturbed =  _deprocess_image( perturbed_processed, preprocessing, color_axis  )
