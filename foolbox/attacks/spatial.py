...
class SpatialAttack(Attack):
    ...
    def __call__(self, input_or_adv, label=None, unpack=True,
                 do_rotations=True, do_translations=True, x_shift_limits=[-5, 5],
                 y_shift_limits=[-5, 5], angular_limits=[-5, 5], granularity=10,
        ...
        def crop_center(img):
            ...
            start = tuple(map(lambda a, da: a//2-da//2, img.shape, a.original_image.shape))
        ...
        for x_shift, y_shift, angle in tqdm(transformations, total=granularity**3):
            ...
            xy_shift = [0, x_shift, y_shift] if a.channel_axis(batch=False) == 0 else [x_shift, y_shift, 0]
        
            # rotate image (increases size)
            x = rotate(a.original_image, angle=angle, axes=axes, reshape=True, order=1)
